export default class Validator {
  ...
  checkRule(ele) {
    let element = ele
      ? document.querySelector('#' + ele).querySelectorAll('.e-validator')
      : document.querySelectorAll('.e-validator')
    let arr = []
    // 遍历数组，获取验证规则
    ...
    for (let [key, v] of new Map(arr.map((v, key) => [key, v]))) {
      ...
      if (l.length === 2) {
        let c =
          this.verify[l[0]].call(this, v.value) &&
          this.verify[l[1]].call(this, v.value)
        obj = { elm, c }
        reArr.push(obj)
      }
    }
    ...
    for (let key of reArr) {
      if (!key.c) {
        element[key.elm].style.cssText =
          'box-shadow: 0 0 1px 2px red inset;outline: none !important;'
      } else {
        le--
      }
    }
  ...
  onlyVerify = e => {
    ...
    if (sp.length === 2)
      status =
        this.verify[sp[0]].call(this, e.value) &&
        this.verify[sp[1]].call(this, e.value)
    if (!status) {
      e.style.cssText =
        'box-shadow: 0 0 1px 2px red inset;outline: none !important;'
    } else {
