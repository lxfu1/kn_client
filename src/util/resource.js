...
import { convertQueryString } from '@/util/utils';
...
axios.interceptors.response.use(
    ...
    function(error) {
        if (
            (error.response && error.response.data && error.response.data.status === 401) ||
            (error.response && error.response.data && error.response.data.status === 403)
        ) {
